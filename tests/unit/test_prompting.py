...
def _r1():
    return Round(
        number=1,
        turns=[
            Turn(elder="ada", answer=_answer("ada", "Ada R1")),
            Turn(elder="kai", answer=_answer("kai", "Kai R1")),
            Turn(elder="mei", answer=_answer("mei", "Mei R1")),
        ],
    )
...
def _r2_with_questions():
    ...
    return Round(
        number=2,
        turns=[
            Turn(elder="ada", answer=_answer("ada", "Ada R2")),
            Turn(elder="kai", answer=_answer("kai", "Kai R2"), questions=(q,)),
            Turn(elder="mei", answer=_answer("mei", "Mei R2"), questions=(q2,)),
        ],
    )
...
class TestBuildSystemMessage:
    def test_empty_when_no_persona_or_context(self, builder):
        assert builder.build_system_message(_debate(), "ada") == ""

    def test_persona_only(self, builder):
        pack = CouncilPack(name="p", shared_context=None, personas={"ada": "You are a lawyer."})
        assert "lawyer" in builder.build_system_message(_debate(pack=pack), "ada")

    def test_shared_context_only(self, builder):
        pack = CouncilPack(name="p", shared_context="You are my chief of staff.", personas={})
        out = builder.build_system_message(_debate(pack=pack), "ada")
        assert "chief of staff" in out

    def test_both_combined(self, builder):
        pack = CouncilPack(name="p", shared_context="Chief.", personas={"ada": "Lawyer."})
        out = builder.build_system_message(_debate(pack=pack), "ada")
    ...
    def test_per_elder_persona_does_not_leak(self, builder):
        pack = CouncilPack(
            name="p",
            shared_context=None,
            personas={"ada": "Lawyer.", "kai": "Engineer."},
        )
        assert "Lawyer" in builder.build_system_message(_debate(pack=pack), "ada")
        assert "Engineer" not in builder.build_system_message(_debate(pack=pack), "ada")
...
class TestBuildRoundTwoUser:
    def test_includes_other_advisors(self, builder):
        out = builder.build_round_2_user(_debate(rounds=[_r1()]), "ada")
        assert "Kai R1" in out
        assert "Mei R1" in out
        assert "Ada R1" not in out  # elder doesn't need to see its own
    ...
    def test_includes_user_messages_when_present(self, builder):
        ...
        out = builder.build_round_2_user(d, "ada")
...
class TestBuildRoundNUser:
    def test_includes_previous_round_other_advisors(self, builder):
        ...
        out = builder.build_round_n_user(d, "ada", 3)
        assert "Kai R2" in out
        assert "Mei R2" in out
    ...
    def test_includes_peer_questions(self, builder):
        d = _debate(rounds=[_r1(), _r2_with_questions()])
        out = builder.build_round_n_user(d, "ada", 3)
        # Ada sees chatgpt→gemini question as peer cross-talk.
...
class TestBuildSynthesis:
    def test_includes_all_rounds(self, builder):
        d = _debate(rounds=[_r1(), _r2_with_questions()])
        out = builder.build_synthesis(d, by="ada")
        assert "Ada R1" in out
        assert "Kai R2" in out
    ...
    def test_anchors_to_user_question_shape(self, builder):
        ...
        out = builder.build_synthesis(_debate("Give me one sentence."), by="ada")
        low = out.lower()
    ...
    def test_severs_length_from_transcript(self, builder):
        ...
        out = builder.build_synthesis(_debate(), by="ada")
        low = out.lower()
