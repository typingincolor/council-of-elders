...
class TestJudgePreference:
    async def test_synthesis_wins_when_x_was_synthesis_and_winner_x(self):
        ...
        v = await judge_preference(
            question="Q?", synthesis="S", best_r1="B",
            judge_port=judge, rng=random.Random(_SEED_X_IS_SYNTHESIS),
        )
    ...
    async def test_best_r1_wins_when_x_was_synthesis_and_winner_y(self):
        judge = FakeElder(elder_id="claude", replies=["winner: Y\nreason: R1 clearer.\n"])
        v = await judge_preference(
            question="Q?", synthesis="S", best_r1="B",
            judge_port=judge, rng=random.Random(_SEED_X_IS_SYNTHESIS),
        )
    ...
    async def test_best_r1_wins_when_x_was_best_r1_and_winner_x(self):
        ...
        v = await judge_preference(
            question="Q?", synthesis="S", best_r1="B",
            judge_port=judge, rng=random.Random(_SEED_X_IS_BEST_R1),
        )
        assert v.winner == "best_r1"
    ...
    async def test_tie_preserved(self):
        judge = FakeElder(elder_id="claude", replies=["winner: TIE\nreason: both ok.\n"])
        v = await judge_preference(
            question="Q?", synthesis="S", best_r1="B",
            judge_port=judge, rng=random.Random(0),
        )
        assert v.winner == "tie"

    async def test_unparseable_reply_defaults_to_tie(self):
        judge = FakeElder(elder_id="claude", replies=["banana\n"])
        v = await judge_preference(
            question="Q?", synthesis="S", best_r1="B",
            judge_port=judge, rng=random.Random(0),
        )
